...
@section('content')
    <div class="pl-3">
        <div class="page-title mb-4">
            <h3>Fire Department Types</h3>
        </div>
        <div class="row mb-4">
            <div class="col-md-6">
                @include('partials.meta-box',['labels'=>['Total Fire Dept. Types:'=>$fire_department_types->count]])
            </div>
            @can('fire_departments.create')
            <div class="col-md-6 text-right">
                <button class="btn btn-primary btn-wd add-fire-department-type"><span class="material-icons">add</span> Add Department Type</button>
            </div>
            @endcan
        </div>
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="id">Fire Dept. Type ID</label>
...
@section('modals')
    ...
    <div id="add-modal" tabindex="1" role="dialog" aria-labelledby="add-modal-title" aria-hidden="true" class="modal fade">
        <div role="document" class="modal-dialog">
            <div class="modal-content">
                <form id="add">
                    <div class="modal-header"><h5 id="add-modal-title" class="modal-title cambria-bold">Add Department Type</h5>
                        <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true">×</span></button>
                    </div>
    ...
    <div id="edit-modal" tabindex="1" role="dialog" aria-labelledby="edit-modal-title" aria-hidden="true" class="modal fade">
        <div role="document" class="modal-dialog">
            <div class="modal-content">
                <form id="edit">
                    <div class="modal-header"><h5 id="edit-modal-title" class="modal-title cambria-bold">Edit Department Type</h5>
                        <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true">×</span></button>
                    </div>
                    <div id="edit-modal-content" class="modal-body">
                        <div class="form-group">
                            ...
                            <input class="form-control" name="description_1">
...
@push('js')
    ...
    <script>
        ...
        function load_records(page, url){
            ...
            url = url ? url : `{{ route('fire-department-type.paginate') }}?${form}&page=${page}`;
        ...
        $('.add-fire-department-type').on('click',function (e) {
            e.preventDefault();
            reset_add_form();
        ...
        $('#add').on('submit', function (e) {
            ...
            axios.post("{{ route('fire-department-type.store') }}",$(this).serialize()).then((response)=>{
        ...
        $(document).on('click','.edit',function (e) {
            ...
            axios.get("{{ route('fire-department-type.index') }}/"+id).then((response)=>{
                console.log(response)
                if(response.data.status){
                    let edit = $('#edit');
                    edit.find('input[name=id]').val(response.data.fire_department_type.id);
                    edit.find('input[name=description_1]').val(response.data.fire_department_type.description);
        ...
        $('#edit').on('submit',function (e) {
            ...
            axios.put("{{ route('fire-department-type.index') }}/"+id,$(this).serialize()).then((response)=>{
                if(response.data.status){
                    reload_current_page();
                    reset_edit_form();
        ...
        function reload_current_page(){
            ...
            url = `{{ route('fire-department-type.paginate') }}?${req}&page=${page}`;
        ...
        *=============================*/
        ...
        $('#delete-form').on('submit',function (e) {
            ...
            axios.delete("{{ route('fire-department-type.index') }}/"+$('[name=delete]').val(),$(this).serialize()).then((response)=>{
                if(response.data.status){
                    Toast.fire({
                        icon: 'success',
                        title: response.data.msg

...
@section('content')
    <div class="pl-3">
        ...
        <div class="row mb-4">
            <div class="col-md-6">
                @include('partials.meta-box',['labels'=>['Total Facility Types:'=>$facility_types->count]])
            </div>
            @can('facilities.create')
            <div class="col-md-6 text-right">
                <button class="btn btn-primary btn-wd add-facility-type"><span class="material-icons">add</span> Add Facility Type</button>
            </div>
            @endcan
        </div>
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="id">Facility Type ID</label>
...
@section('modals')
    ...
    <div id="add-modal" tabindex="1" role="dialog" aria-labelledby="add-modal-title" aria-hidden="true" class="modal fade">
        <div role="document" class="modal-dialog">
            <div class="modal-content">
                <form id="add">
                    <div class="modal-header"><h5 id="add-modal-title" class="modal-title cambria-bold">Add Facility Type</h5>
                        <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true">×</span></button>
                    </div>
    ...
    <div id="edit-modal" tabindex="1" role="dialog" aria-labelledby="edit-modal-title" aria-hidden="true" class="modal fade">
        <div role="document" class="modal-dialog">
            <div class="modal-content">
                <form id="edit">
                    <div class="modal-header"><h5 id="edit-modal-title" class="modal-title cambria-bold">Edit Facility Type</h5>
                        <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true">×</span></button>
                    </div>
                    <div id="edit-modal-content" class="modal-body">
                        <div class="form-group">
                            ...
                            <input class="form-control" name="description_1" style="text-transform: capitalize;">
...
@push('js')
    ...
    <script>
        ...
        function load_records(page, url){
            ...
            url = url ? url : `{{ route('facility-type.paginate') }}?${form}&page=${page}`;
        ...
        $('#add').on('submit', function (e) {
            ...
            axios.post("{{ route('facility-type.store') }}",$(this).serialize()).then((response)=>{
        ...
        $(document).on('click','.edit',function (e) {
            ...
            edit.find('input[name=description_1]').val(description);
            $('#edit-modal').modal('show');

            // axios.get("{{ route('facility-type.index') }}/"+id).then((response)=>{
        ...
        $('#edit').on('submit',function (e) {
            ...
            axios.put("{{ route('facility-type.index') }}/"+id,$(this).serialize()).then((response)=>{
                if(response.data.status){
                    reload_current_page();
                    reset_edit_form();
        ...
        function reload_current_page(){
            ...
            url = `{{ route('facility-type.paginate') }}?${req}&page=${page}`;
        ...
        *=============================*/
        ...
        $('#delete-form').on('submit',function (e) {
            ...
            axios.delete("{{ route('facility-type.index') }}/"+$('[name=delete]').val(),$(this).serialize()).then((response)=>{
                if(response.data.status){
                    Toast.fire({
                        icon: 'success',
                        title: response.data.msg

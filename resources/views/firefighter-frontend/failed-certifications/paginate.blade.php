    ...
    @if($firefighter_certificates && $firefighter_certificates->count())
        @foreach($firefighter_certificates as $firefighter_certificate)
                <tr style="{{ $firefighter_certificate->read_status == 0 ? "background-color:rgba(0,0,0,.075);" : '' }}">
                    ...
                    <td class="text-capitalize">{{ $firefighter_certificate->firefighter_certificates_status }}</td>
                    <td>
                        <a href="{{ route('firefighters.failed-certificates.show',$firefighter_certificate->id) }}"  title="View"><span class="material-icons">visibility</span></a>
                    </td>
                </tr>
        @endforeach
    @else

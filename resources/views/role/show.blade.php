...
@section('content')
    <div class="pl-3">
        ...
        <div class="row">
            <div class="col-md-6">
                <span class="segoe-ui-italic">Roles & Permissions > View Role</span>
            </div>
        </div>
        <div class="row mb-5">
                            ...
                            @foreach($permissions as $key=>$permission)
                                ...
                                <div class="col-md-5">
                                    <table class="table table-borderless w-100">
                                        <tbody>
                                        <tr>
                                            <th width="150" class="text-capitalize"><label>{{ str_replace('_',' ',$single_permission) }}</label></th>
                                            <td>
                                                <div class="show-field">
                                                    <table class="mb-0 table-sm">
                                                        <tr>
                                                            <th width="110"><label><span class="material-icons">add</span> Create:</label></th>
                                                            <td>{{ $role->hasPermissionTo("{$single_permission}.create") ? 'Yes' : 'No' }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th><label><span class="material-icons">visibility</span> View:</label></th>
                                                            <td>{{ $role->hasPermissionTo("{$single_permission}.read") ? 'Yes' : 'No' }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th><label><span class="material-icons">edit</span> Update:</label></th>
                                                            <td>{{ $role->hasPermissionTo("{$single_permission}.update") ? 'Yes' : 'No' }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th><label><span class="material-icons">delete</span> Delete:</label></th>
                                                            <td>{{ $role->hasPermissionTo("{$single_permission}.delete") ? 'Yes' : 'No' }}</td>
                                                        </tr>
                                                    </table>
                                                </div>
                                                <div class="edit-field d-none text-left">
                                                    <div class="custom-control custom-switch">
                                                        <input type="checkbox" class="custom-control-input" id="{{$single_permission}}.create" {{ $role->hasPermissionTo("{$single_permission}.create") ? 'checked' : '' }} name="{{$single_permission}}[create]" value="1">
                                                        <label class="custom-control-label" for="{{$single_permission}}.create">Create</label>
                                                    </div>
                                                    <div class="custom-control custom-switch">
                                                        <input type="checkbox" class="custom-control-input" id="{{$single_permission}}.read" {{ $role->hasPermissionTo("{$single_permission}.read") ? 'checked' : '' }} name="{{$single_permission}}[read]" value="1">
                                                        <label class="custom-control-label" for="{{$single_permission}}.read">View</label>
                                                    </div>
                                                    <div class="custom-control custom-switch">
                                                        <input type="checkbox" class="custom-control-input" id="{{$single_permission}}.update" {{ $role->hasPermissionTo("{$single_permission}.update") ? 'checked' : '' }} name="{{$single_permission}}[update]" value="1">
                                                        <label class="custom-control-label" for="{{$single_permission}}.update">Update</label>
                                                    </div>
                                                    <div class="custom-control custom-switch">
                                                        <input type="checkbox" class="custom-control-input" id="{{$single_permission}}.delete" {{ $role->hasPermissionTo("{$single_permission}.delete") ? 'checked' : '' }} name="{{$single_permission}}[delete]" value="1">
...
@push('js')
    ...
    <script type="text/javascript">
        ...
        $('#add').on('submit', function (e) {
            ...
            axios.post("{{ route('role.update',$role->id) }}",$(this).serialize()).then((response)=>{
